...
function nwsetupmatrix(s1, s2)
    return zeros(Int, length(s2)+1, length(s2)+1)
end
...
#function swsetupmatrix(seq1, seq2)
   # setupmatrix= zeros(Int, length(s1)+1), length(seq2)+1
   # end
   # scoremat[i, j] = (above, left, diagonal, 0)
#end
#return setupmatrix
#end